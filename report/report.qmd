...
ggplot(data = filter(combined_aligned_stats, category == "total"), aes(y = barcode, x = QC_pass)) +
    geom_col()
```
...
# Combine datasets together and rename columns
combined_counts <- rbindlist(counts_list, idcol = "barcode", fill = TRUE)
...
combined_counts %>%
    ...
    mutate(barcode = fct_reorder(barcode, count)) %>%
    ggplot(aes(y = barcode, x = count)) +
    geom_col()
...
combined_counts_prop <- combined_counts_prop %>%
    ...
    ungroup() %>%
    select(-prop) %>%
    right_join(combined_counts_prop, by = join_by(barcode, sequence)) %>%

...
ggplot(
    data = filter(combined_aligned_stats, category == "total"),
...
counts_files <- list.files(
    path = "./results/2. extracted reads/best/counts",
    pattern = "\\.csv$",
    full.names = TRUE
)
...
combined_counts_prop <- combined_counts_prop %>%
    ...
    mutate(sequence = factor(sequence, levels = c("total reads", "rotated", "heavy", "heavy + kappa", "heavy + lambda")))


combined_counts_prop %>%
    ggplot() +
    geom_col(
        aes(y = fct_reorder(sample_id, count), x = diff, fill = fct_rev(sequence)),
        position = "stack"
    ) + 
    guides(fill = guide_legend(reverse = TRUE)) +
    scale_fill_manual(values = c("total reads" = "#31eb3af4",
                                "rotated" = "#faf742ca",
                                "heavy" = "#ed9325ff",
                                "heavy + kappa" = "#51daf9c2",
                                "heavy + lambda" = "#45b7d187")) +
